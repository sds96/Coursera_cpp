...
StatsData BuildStatsData(vector<Person> people) {
...

  {
    ...
    // ѕо мере обработки запросов список людей не мен€етс€, так что мы можем
    // один раз найти самые попул€рные женское и мужское имена
    result.most_popular_male_name = FindMostPopularName(males);
    result.most_popular_female_name = FindMostPopularName(females);
  }

  // «апросы WEALTHY можно тоже обрабатывать за ќ(1), один раз отсортировав всех
