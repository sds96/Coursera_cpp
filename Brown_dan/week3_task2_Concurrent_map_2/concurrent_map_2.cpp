...
public:
  ...
  explicit ConcurrentMap(size_t bucket_count): vec_data(bucket_count) {
  };

  WriteAccess operator[](const K& key){
	  size_t ind = hasher(key)%vec_data.size();
	  return { lock_guard<mutex>(vec_data[ind].m), vec_data[ind].data[key] };
  };
  ReadAccess At(const K& key) const{
	  size_t ind = hasher(key)%vec_data.size();
	  try {
		  const auto& value = vec_data[ind].data.at(key);
		  return { lock_guard<mutex>(vec_data[ind].m), value };
	  }
  ...
  MapType BuildOrdinaryMap() const{
	  ...
	  for(auto& dat: vec_data){
		  lock_guard<mutex> g(dat.m);
		  for(const auto& el: dat.data){
