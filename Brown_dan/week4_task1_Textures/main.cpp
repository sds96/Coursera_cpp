...
void TestShapeBiggerCanvas() {
  Canvas canvas;
  canvas.SetSize({5, 3});

  canvas.AddShape(ShapeType::Rectangle, {1, 0}, {7, 3}, nullptr);
...
void Test2Fig1Text() {
  ...
  auto p = canvas.AddShape(ShapeType::Rectangle, {0, 0}, {3, 1},
		  	  	  	  	  	  MakeTextureSolid({2, 2}, '+'));

  canvas.DuplicateShape(p, {0,2});
...
void TestTextureBiggerShape() {
  Canvas canvas;
  canvas.SetSize({6, 4});

  canvas.AddShape(ShapeType::Rectangle, {1, 1}, {4, 2},
                  MakeTextureSolid({3, 4}, '*'));
...
int main() {
  ...
  RUN_TEST(tr, TestShapeBiggerCanvas);
  RUN_TEST(tr, Test2Fig1Text);
  RUN_TEST(tr, TestSmallTexture);
  RUN_TEST(tr, TestTextureBiggerShape);
  RUN_TEST(tr, TestCow);
  RUN_TEST(tr, TestCpp);
  return 0;
}
